...
import {CommonLoadingPopup, CreateUUID, deleteUploadFile, isValidData, TRAINING_KEYWORD} from "../CommonMethods";
...
export default class BrowseFile extends React.Component {
    ...
    componentWillReceiveProps(nextProps, nextContext) {
        // console.log("#BrowseFile componentWillReceiveProps: ", nextProps)
            ...
            if (acceptedFiles.length !== 0 && acceptedFiles.length <= limitCount) {
                ...
                acceptedFiles.forEach(file => {
                    if (isValidData(file)) {
                        ...
                        let singleFileName = file.name
                        let splitArr = file.name.split(".")
                        let originalFileName = splitArr[0]
                        let singleEngine = splitArr[1]

                        let newStatusID = CreateUUID()
                        let uploadFileID = newStatusID.replaceAll("-", "_")
                ...
                if (this.props.reinforceFile === true) {
                    if (engine === "csv" || engine === "xlsx" || engine === "json") {
                        ...
                        let params = "?modelID=" + this.state.modelID + "&clientID=" + this.state.clientID
                        renderSwitchPatentService("UploadFileForReinforceData", data, params, '', requestID).then(response => {
                ...
                } else if (this.props.patentFile === true) {
                    if (engine === "csv" || engine === "xlsx" || engine === "json") {
                        this.setState({
                            commonLoading: false
                        })
                        this.props.handleSubmitFile(data, uploadedFileInfos)
                    } else {
                        this.setState({
                            commonLoading: false,
                            defaultErrorPopup: true,
    ...
    firstRowAsHeaderInfo(fileInfo, header) {
        ...
        let requestID = CreateUUID()
        renderSwitchRoe("firstRowAsHeaderInfo", passData, '', '', requestID).then(response => {
                console.log("response of get first row as header: ", response.data);
                let headerResults = response.data.Results
                if (response.data["ResponseCode"] === 1000) {
                    ...
                    let defaultErrorText = "Failed to upload - invalid data format!"
                    if (response.data["ResponseCode"] === 3666) {
                        defaultErrorText = "Some header column names are empty string. Please give the correct header name."
                    }
                    this.setState({
                        commonLoading: false,
                        defaultErrorPopup: true,
                    ...
                    deleteUploadFile(fileInfo.TableName)
                }
            }
        ).catch((e) => {
            ...
            <>
                {
                    fileDropStatus === "initial" ?
                        <div className="default-file-drop-zone"
                             style={{boxShadow: noShadow ? "none" : "0px 0.36px 0.8px 0px rgba(0, 0, 0, 0.12), 0px 1.6px 3px 0px rgba(0, 0, 0, 0.16)"}}>
                            {
                                        ...
                                        <div {...getRootProps()}
                                            ...
                                            {/*       onChange={this.handleSaveFileInfo.bind(this)}/>*/}
                                            {
                                                ...
                                                <>
                                                    ...
                                                    {
                                                        isValidData(limitationNode2) &&
                                                        <div className="default-limit-note">{limitationNode2}</div>
                                                    }
                                                </>
                                            }
                ...
                {
                    this.state.openPopupForShareConfirmation === true ?
                        <ResponsePopup
                            open={true}
                            type={"confirmation"}
                            title={"Confirmation"}
                            bodyContext={this.state.confirmationMessage}
                            cancelBtn={true}
                            primaryButtonText={"Ok"}
                            secondaryButtonText={"Cancel"}
                            onClose={() => this.handleClosePopup("openPopupForShareConfirmation")}
                            onSubmit={() => this.handleSubmitForShareConfirmation()}
                        />
                        :
                        null
                }
...
BrowseFile.defaultProps = {
    acceptFiles: ".csv,.xlsx,.json",
    selectedFileUsage: TRAINING_KEYWORD,
    limitation: true,
    noShadow: false,
